        ...
        <ul class="list-inline">
            <li class="list-inline-item">
                <h{{ isset($headerSize) && $headerSize ? $headerSize : '1' }} class="mb-0 font-weight-bold">
                    {{ isset($titlePrefix) && $titlePrefix ? $titlePrefix : '' }}<a href="{{ route('member.show', ['guildSlug' => $guild->slug, 'usernameSlug' => $member->slug]) }}" class="text-white">{{ $member->username }}</a>{{ isset($titleSuffix) && $titleSuffix ? $titleSuffix : '' }}
                </h{{ isset($headerSize) && $headerSize ? $headerSize : '1' }}>
            </li>
            @if (isset($showEdit) && $showEdit)
                <li class="list-inline-item">
                    <a href="{{ route('member.edit', ['guildSlug' => $guild->slug, 'usernameSlug' => $member->slug]) }}">
