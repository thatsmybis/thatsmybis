...
@php
    ...
    foreach ($instance->itemSources as $itemSource) {
        $itemSourceAmounts[$itemSource->name] = [
            'wishlistCount' => $guild->is_wishlist_disabled ? 0 : array_sum(array_keys($items->where('source_name', $itemSource->name)->countBy(function ($source) { return $source->wishlistCharacters->where('pivot.is_received', 0)->count();})->toArray())),
            'prioCount'     => $guild->is_prio_disabled ? 0 : array_sum(array_keys($items->where('source_name', $itemSource->name)->countBy(function ($source) { return $source->priodCharacters->where('pivot.is_received', 0)->count();})->toArray())),
        ];
    }
@endphp
