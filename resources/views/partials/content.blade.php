...
<div class="js-content bg-light mb-1 p-2 rounded" data-id="{{ $content->id }}" style="display:none;">
    <form class="form-horizontal" role="form" method="POST" action="{{ route('updateContent', $content->id) }}">
        ...
        <div class="">
            <small class="text-muted">{{ __("format text with") }} <a target="_blank" href="{{ env('LINK_MARKDOWN') }}">{{ __("markdown") }}</a></small>
        </div>
...
<div>
    <ul class="list-inline mb-0 text-muted">
        @if ($content->category != 'resource')
            ...
            <li class="small list-inline-item">
                <span class="js-watchable-timestamp" data-timestamp="{{ strtotime($content->created_at) }}"></span> {{ __("ago") }}
            </li>
        @endif
        @if (Auth::user()->hasRole(env('PERMISSION_RAID_LEADER')))
            <li class="small list-inline-item">
                <a class="js-edit-content" data-id="{{ $content->id }}" href="">{{ __("edit") }}</a>
            </li>
            <li class="small list-inline-item">
                <form id="removeContent{{ $content->id }}" class="form-horizontal" role="form" method="POST" action="{{ route('removeContent', $content->id) }}">
                    {{ csrf_field() }}
                    <button class="link" onClick="return confirm('Are you sure you want to remove this content?');">{{ __("remove") }}</button>
                </form>
            </li>
        @endif
    </ul>
