...
Rrun `sudo systemctl enable redis-server` to tell the server to boot on system startup. If you're getting a `connection refused` error in the app, it may be because the Redis server isn't running.
...
- `max_input_vars` in `php.ini` (PHP's config) has been increased from 1000 to 3000. This is to support some pages with an absurd amount of inputs. (ie. 120 items with 20 input fields each = 2400 inputs)
