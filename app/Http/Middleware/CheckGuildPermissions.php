    ...
    public function handle($request, Closure $next)
    {
        if ($request->route('guildId') !== null) {
            ...
            if ($currentMember && ($currentMember->banned_at || $currentMember->inactive_at) && !$isAdmin) {
                request()->session()->flash('status-danger',  __('Your membership has been disabled. To reverse this, an officer would need to access your member page and re-enable it.') . ' ' . __('They can do this in Guild -> Members -> Archived Members (bottom left of page) -> Edit your profile -> Uncheck box to archive'));
                return redirect()->route('home');
            }
